...
function validateForm() {
    ...
    for(var i=0;i<allList.length ;i++){
        ...
        var newError = $('<div class="errormsg">Please fill this</div>');
        ...
        if (element.val() === "" && element.next() && element.next().hasClass('err')) {
            ...
            flag = false;
        } else if (element.next() && element.next().hasClass('err')) {
           element.next().remove();
        }
    }
    ...
    if (pno.val().length !== 10) {
        var newError = $('<div class="errormsg">Phone number should be 10 digit</div>');
        var errDiv = $('<div class="err"></div>').append(newError);

        if (pno.next().hasClass('err')) {
            pno.next().remove();
        }
    ...
    if (pincode.val().length !== 6) {
        var newError = $('<div class="errormsg">Pincode must be in 6 digits</div>');
        var errDiv = $('<div class="err"></div>').append(newError);

        if (pincode.next().hasClass('err')) {
            pincode.next().remove();
        }
    ...
    if (!emailPattern.test(email)) {
        var newError = $('<div class="errormsg">Can\'t be an email</div>');
        var errDiv = $('<div class="err"></div>').append(newError);

        if ($("#txtEmail").next().hasClass('err')) {
